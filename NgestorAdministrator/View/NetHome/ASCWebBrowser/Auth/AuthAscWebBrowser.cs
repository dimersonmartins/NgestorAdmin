...
namespace NgestorAdministrator.View.NetHome.ASCWebBrowser.Auth
{
    public class AuthAscWebBrowser
    {
        public async Task<bool> execute()
        {
            Config.ASC_KEY_REMOTE   = GenereteKey(8);
            Config._JSESSIONID      = await loginASC();
            return true;
        }
        ...
        public async Task<string> loginASC()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                try
                {
                    ...
                    return resposta.Substring(postion_incial, 75).Replace("Set-Cookie: ", "");
                }
                catch (Exception)
                {
                    return "[]";
